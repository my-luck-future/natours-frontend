...
{
path: '/', 用来接收路由
loader: tourListLoader, 在渲染组件之前加载接口数据
element: <TourList />, loader 加载数据成功后渲染正常页面
errorElement: <Error />, loader 报错跳转 error 页面友好提示用户
}

TourList.jsx 展示 9 条旅游路线界面：使用 useLoaderData 获取 tour 列表

Tour.jsx 旅游路线详情界面：使用 useLoaderData 获取点击 tour 详情，useSelector 获取登录 user 信息，展示当前旅行路线的简介、图片、地图坐标、历史评论、预定按钮

- useSelector 函数：从 Redux store 中订阅并提取所需的状态数据，当数据变化时自动触发组件重渲染
...
MyTours.jsx 展示 user 订购的 Tour 界面：使用 useLoaderData 获取当前用户订购的 tour 路线，前端不传递 userId，由后端 nodejs 通过 protect 中间件解析 header/token/cookie 参数得到
